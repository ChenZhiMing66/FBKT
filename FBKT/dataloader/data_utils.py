...
def set_up_datasets(args):
    if args.dataset == 'cifar100':
        ...
        args.base_class = 96  # Assuming some base class number
        args.num_classes = 196  # stanfordcar has 196 classes
        args.way = 10
        args.shot = 5
        args.sessions = 11
...
def get_base_dataloader(args):
    ...
    if args.dataset == 'stanfordcar':
        trainset = args.Dataset.StanfordCarDataset(root=args.dataroot+'/Stanford_Car/', session=1, train=True, transform=None, crop_transform=crop_transform, secondary_transform=secondary_transform)
        testset = args.Dataset.StanfordCarDataset(root=args.dataroot+'/Stanford_Car/', session=1, train=False, transform=None)
...
def get_new_dataloader(args,session):
    ...
    if args.dataset == 'mini_imagenet':
        trainset = args.Dataset.MiniImageNet(root=args.dataroot, train=True, index_path=txt_path,
                                             crop_transform=crop_transform, secondary_transform=secondary_transform)
    if args.dataset == 'stanfordcar':
        trainset = args.Dataset.StanfordCarDataset(root=args.dataroot+'/Stanford_Car/', session=session + 1, train=True, transform=None, crop_transform=crop_transform, secondary_transform=secondary_transform)
    ...
    if args.dataset == 'mini_imagenet':
        testset = args.Dataset.MiniImageNet(root=args.dataroot, train=False,
                                      index=class_new)
    if args.dataset == 'stanfordcar':
        testset = args.Dataset.StanfordCarDataset(root=args.dataroot+'/Stanford_Car/', session=session + 1, train=False, transform=None)
