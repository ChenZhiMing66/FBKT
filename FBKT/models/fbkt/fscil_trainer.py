...
from losses import SupContrastive, ClassContrastive
    ...
    def train(self):
        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:  # load base class train img label
                ...
                class_criterion = ClassContrastive()
                class_criterion = class_criterion.cuda()

                for epoch in range(args.epochs_base):
                    start_time = time.time()
                    # train base sess
                    tl, tl_mask, tl_joint, tl_class_loss, tl_IAC, ta = base_train(self.model, trainloader, criterion, class_criterion, optimizer, scheduler, epoch, self.transform, args)
                    ...
                    result_list.append(
'epoch:%03d,lr:%.4f,training_loss:%.5f,joint_loss:%.5f, tl_class_loss:%.5f, kl_loss:%.5f, IAC_loss:%.5f, training_acc:%.5f,test_loss:%.5f,test_acc:%.5f'% (epoch, lrc, tl, tl_joint, tl_class_loss, tl_mask, tl_IAC, ta, tsl, tsa))
                ...
                if args.incft:
                    self.pre_model.load_state_dict(self.best_model_dict, strict=True)
                    self.pre_model.mode = self.args.new_mode
                    self.pre_model.eval()
                    self.pre_model.fc.weight.data = self.model.fc.weight.clone().data

                if args.incft:
                    trainloader.dataset.transform = train_transform
                    train_set.multi_train = True
                    for epoch in range(args.epochs_new):
                        update_fc_ft(trainloader, self.transform, self.model, self.pre_model, self.num_trans, session, args, epoch)
