...
class MYNET(nn.Module):
    def __init__(self, args, mode=None, trans=1):
        ...
        if self.args.dataset in ['cifar100']:
            self.encoder_q = resnet20(num_classes=32)
            self.encoder_k = resnet20(num_classes=32)
            self.num_features = 64
            self.queue_dim = 32
        if self.args.dataset in ['mini_imagenet']:
            self.encoder_q = resnet18(True, args, num_classes=128)
            self.encoder_k = resnet18(True, args, num_classes=128)
            self.num_features = 512
            self.queue_dim = 128
        if self.args.dataset == 'cub200' or self.args.dataset == 'stanfordcar':
            self.encoder_q = resnet18(True, args, num_classes=128)
            self.encoder_k = resnet18(True, args, num_classes=128)
            self.num_features = 512
            self.queue_dim = 128
        ...
        self.mask_encoder = copy.deepcopy(self.encoder_q)

        for param_q, param_k in zip(self.encoder_q.parameters(), self.encoder_k.parameters()):
            param_k.data.copy_(param_q.data)  # initialize
            param_k.requires_grad = False  # not update by gradient

        for param_q, param_m in zip(self.encoder_q.parameters(), self.mask_encoder.parameters()):
            param_m.data.copy_(param_q.data)  # initialize
            param_m.requires_grad = False  # not update by gradient

        # create the queue
        self.register_buffer("queue", torch.randn(self.queue_dim, self.K))
    ...
    @torch.no_grad()
    def _momentum_update_mask_encoder(self, base_sess):
        """
        Momentum update of the key encoder
        """
        # mask_m = 0.9
        if base_sess:
            for param_q, param_m in zip(self.encoder_q.parameters(), self.mask_encoder.parameters()):
                param_m.data = param_q.data
                param_m.requires_grad = False
        else:
            for k, v in self.encoder_q.named_parameters():
                if k.startswith('fc') or k.startswith('layer4') or k.startswith('layer3'):
                    self.mask_encoder.state_dict()[k].data = v.data
    ...
    def forward_aug(self, x):
        ...
        return x # class contrastive

    def forward_mask(self, x):
        x, _ = self.encode_mask(x)
        if 'cos' in self.mode:
            x = F.linear(F.normalize(x, p=2, dim=-1), F.normalize(self.fc.weight, p=2, dim=-1))
    ...
    def forward(self, im_cla, im_aug=None, im_mask=None, im_q=None, im_k=None, labels=None, im_q_small=None, base_sess=True,
        ...
        if self.mode != 'encoder':
            if im_q == None:
                ...
                with torch.no_grad():  # no gradient to mask
                    self._momentum_update_mask_encoder(base_sess)  # update the mask encoder
                    mask_logits_classify = self.forward_mask(im_mask)
                ...
                if im_aug is not None:
                    with torch.no_grad():  # no gradient to mask
                        aug_logits_classify = self.forward_aug(im_aug)
                else:
                    aug_logits_classify = None
                ...
                return logits_classify, aug_logits_classify, mask_logits_classify, logits_global, logits_small, targets_global, targets_small
